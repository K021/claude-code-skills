...
>
> **Bonus — refresh before auto-compaction.** `project-context` can also refresh itself *before* the context window compacts: a small statusline sensor (`statusline/ctx-sensor.sh`) feeds the exact context-usage % to a `Stop`/`PostToolBatch` hook that forces an update once you near the limit — so state is saved while it still exists. Optional, interactive-session only; see [project-context → Context-aware refresh](./skills/project-context/#context-aware-refresh-optional-but-recommended).
