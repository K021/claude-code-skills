...
>
> **Bonus — refresh before auto-compaction.** Both skills can also refresh themselves *before* the context window compacts: a small shared statusline sensor (`statusline/ctx-sensor.sh`) feeds the exact context-usage % to each skill's `Stop`/`PostToolBatch` hook, which forces an update once you near the limit — so state is saved while it still exists. Each also snapshots its file via a `PreCompact` hook as a backstop. Optional, interactive-session only; see [project-context → Context-aware refresh](./skills/project-context/#context-aware-refresh-optional-but-recommended) (and the same section in [project-structure](./skills/project-structure/#context-aware-refresh-optional-but-recommended)).
